...
public class DescontoGerencialFieldConverter extends BigDecimalConverter {
	@Override
	...
	private String buildButtonScript(EFormField field) {
		...
		sb.append("				<script>" + "					function processaDesconto(){"
				+ "						const desconto = $(\"#var_DesGerWPH__\").val().replace('%','');"
				...
				+ "							fetch(`custom/desconto_pedido_hidroponia_gerencial.jsp?pedidoId=${pedidoId}&desconto=${desconto}`).then(function(response) {"
				+ "							  if(response.ok) {"
				+ "								$('input[name=\"action.send\"]').length ==1? $('input[name=\"action.save\"]').click() : $('input[name=\"action.apply\"]').click();"
				+ "							  } else {"
